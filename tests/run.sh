...
build_dir="$(mktemp -d "${TMPDIR:-/tmp}/zmk-input-abs2rel-test.XXXXXX")"
...
printf '#include <zmk-input-abs2rel/absolute_to_relative_core.h>\n' >"$build_dir/header.c"
...
for variant in "${variants[@]}"; do
    ...
    cc "${warnings[@]}" "${flags[@]}" -I"$repo_root/include" \
        "$repo_root/tests/test_absolute_to_relative_core.c" \
